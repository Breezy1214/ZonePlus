...
function Zone:_startConsolidatedPartHeartbeat()
	-- Start consolidated heartbeat if not already running
	if self._partHeartbeatConnection then
		return
	end

	local trackingDict = self.trackingTouchedTriggers["part"]
	self._partHeartbeatConnection = heartbeat:Connect(function()
		local clockTime = os.clock()
		local cooldown = enum.Accuracy.getProperty(self.accuracy)

		for part, partData in pairs(trackingDict) do
			local success, err = pcall(function()
				if clockTime >= partData.nextCheck then
					partData.nextCheck = clockTime + cooldown
					...
					if not partData.verifiedEntrance then
						if withinZone then
							partData.verifiedEntrance = true
							self.partEntered:Fire(part)
						elseif
							(part.Position - partData.enterPosition).Magnitude > 1.5
							and clockTime - partData.enterTime >= cooldown
						then
					...
					elseif not withinZone then
						...
						partData.enterPosition = part.Position
						partData.enterTime = os.clock()
...
function Zone:_stopConsolidatedPartHeartbeat()
	if self._partHeartbeatConnection then
		self._partHeartbeatConnection:Disconnect()
		self._partHeartbeatConnection = nil
	end
end
...
function Zone:_partTouchedZone(part)
	...
	self.totalPartVolume = self.totalPartVolume + partVolume

	-- Store part metadata in trackingDict
	trackingDict[part] = {
		janitor = partJanitor,
		nextCheck = 0,
	...
	-- Start consolidated heartbeat if this is the first part
	local partCount = 0
	for _ in pairs(trackingDict) do
	...
	partJanitor:Add(function()
		...
		-- Stop consolidated heartbeat if this was the last part
...
function Zone:_onItemCallback(eventName, desiredValue, instance, callbackFunction)
	...
	table.insert(detail, callbackId)

	local function triggerCallback()
		callbackFunction()

		-- Clean up this specific callback from detail
		local currentDetail = self.onItemDetails[instance]
		if currentDetail then
			for i = #currentDetail, 1, -1 do
				if currentDetail[i] == callbackId then
					table.remove(currentDetail, i)
					break
				end
			end
			-- If no more callbacks, clean up the entry
			if #currentDetail == 0 then
				self.onItemDetails[instance] = nil
				if self.itemsToUntrack[instance] then
					self.itemsToUntrack[instance] = nil
					self:untrackItem(instance)
				end
